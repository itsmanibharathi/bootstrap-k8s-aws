...
# 🔧 Config
JUMPBOX_KEY="./ssh/jumpbox-key"
CONTROL_KEY="./ssh/controlnode-key"
WORKER_KEY="./ssh/workernode-key"
...

{
  ...
  for i in "${!CONTROL_NAMES[@]}"; do
    NAME="${CONTROL_NAMES[$i]} cp-$((i+1))"
    IP="${CONTROL_IPS[$i]}"
    echo "Host $NAME"
    echo "    HostName $IP"
    echo "    User root"
    echo "    IdentityFile ~/.ssh/controlnode-key"
    echo "    StrictHostKeyChecking no"
    echo ""
  done
  ...
  if [ "${#WORKER_NAMES[@]}" -gt 0 ]; then
    ...
    for i in "${!WORKER_NAMES[@]}"; do
      ...
      echo "    HostName $IP"
      echo "    User root"
      echo "    IdentityFile ~/.ssh/workernode-key"
      echo "    StrictHostKeyChecking no"
      echo ""
    done
  fi
...
# create the file under the ./ssh/config
cp "$CONFIG_FILE" ./ssh/config
